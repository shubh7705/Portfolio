...
export default function Skills() {
  return (
    <section id="skills" className="section">
      ...
      <p className="mb-10 max-w-2xl text-slate-400 text-sm leading-relaxed -mt-6">
          ...
          >
            <div>
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="font-mono text-[11px] text-slate-500">{group.skills.length} skills</span>
              </div>
              <h3 className="text-base font-semibold text-slate-100 group-hover:text-accent transition-colors">
                {group.title}
              </h3>
              <p className="mt-1 text-xs text-slate-400 leading-relaxed mb-4">
                {group.description}
              </p>
            </div>

            <ul className="flex flex-wrap gap-2 pt-2 border-t border-slate-800/80">
              {group.skills.map((skill) => (
                <li
                  key={skill}
                  className="rounded-md border border-slate-700/60 bg-base/80 px-2.5 py-1 text-xs font-mono text-slate-300 transition-colors hover:border-accent/60 hover:text-white"
                >
                  {skill}
                </li>
              ))}
